...
public class View {
	...
	TableColumn<RecommendedNutrient, String> recommendedNutrientQuantityColumn = new TableColumn<>(
			"Recommended \nquantity per day");
	...
	TableColumn<Product.ProductNutrient, String> productNutrientNameColumn = new TableColumn<>("Product Nutrients"); // uses
																														// Callback
																														// to
																														// get
																														// nutrientName
																														// from
																														// Model.nutrients
	TableColumn<Product.ProductNutrient, String> productNutrientQuantityColumn = new TableColumn<>(
			"Quantity\n/ 100 Product UOM"); // uses callback to calculate nutrient-quantity
	TableColumn<Product.ProductNutrient, String> productNutrientUomColumn = new TableColumn<>("Nutrient UOM"); // uses
																												// callback
																												// to
																												// get
																												// nutrient
	...
	void setupMenus() {
		...
		nutriProfileMenu.getItems().addAll(newNutriProfileMenuItem, openNutriProfileMenuItem, saveNutriProfileMenuItem,
				new SeparatorMenuItem(), closeNutriProfileMenuItem, exitNutriProfileMenuItem);
	...
	void setupNutriTrackerGrid() {
		...
		productNutrientsTableView.getColumns().setAll(productNutrientNameColumn, productNutrientQuantityColumn,
				productNutrientUomColumn);
		...
		// set seven columns' width: product search field, nutrient search field, search
		// result, search button x 2, clear button x 2
		for (int i = 0; i < 7; i++) {
			productGrid.getColumnConstraints().add(new ColumnConstraints(123));
		}

		dietProductsTableView.getColumns().addAll(dietProductNameColumn, dietServingSizeColumn, dietServingUomColumn,
				dietHouseholdSizeColumn, dietHouseholdUomColumn);
		...
		productIngredientsTextArea.setBorder(new Border(
	...
	void setupNutriProfilerGrid() {
		...
		recommendedNutrientsTableView.getColumns().setAll(recommendedNutrientNameColumn,
				recommendedNutrientQuantityColumn, recommendedNutrientUomColumn);
	...
	void initializePrompts() {
		...
		ObservableList<String> physicalActivityOptions = FXCollections.observableArrayList("Sedentary", "Low active",
				"Active", "Very active");
