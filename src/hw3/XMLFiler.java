	...
	public boolean readFile(String filename) {
		...
		if (profileElement != null) {
			...
			if (ingredientsToWatch.length() > 0) {
				ingredientsToWatch.replace(ingredientsToWatch.toString().length() - 2,
						ingredientsToWatch.toString().length() - 1, "");
			}
			switch (gender) {
			case "Female":
				NutriByte.person = new Female(age, weight, height, physicalActivityLevel,
						ingredientsToWatch.toString());
				break;
			case "Male":
