...
public class NutriChart {
	...
	void updateChart() {
		if (NutriByte.person != null) {
			...
			for (RecommendedNutrient rn : NutriByte.person.recommendedNutrientsList) {
				if (rn.getNutrientCode().equals(NutriProfiler.ENERGY_NUTRIENT_CODE)
						&& NutriByte.person.dietNutrientsMap.containsKey(rn.getNutrientCode())) {
					actualDataList.get(0).setYValue(
							Math.round(NutriByte.person.dietNutrientsMap.get(rn.getNutrientCode()).getNutrientQuantity()
									/ rn.getNutrientQuantity() * 100));
				}
				if (rn.getNutrientCode().equals(NutriProfiler.NutriEnum.PROTEIN.getNutrientCode())
						&& NutriByte.person.dietNutrientsMap.containsKey(rn.getNutrientCode())) {
					actualDataList.get(1).setYValue(
							Math.round(NutriByte.person.dietNutrientsMap.get(rn.getNutrientCode()).getNutrientQuantity()
									/ rn.getNutrientQuantity() * 100));
				}
				if (rn.getNutrientCode().equals(NutriProfiler.NutriEnum.CARBOHYDRATE.getNutrientCode())
						&& NutriByte.person.dietNutrientsMap.containsKey(rn.getNutrientCode())) {
					actualDataList.get(2).setYValue(
							Math.round(NutriByte.person.dietNutrientsMap.get(rn.getNutrientCode()).getNutrientQuantity()
									/ rn.getNutrientQuantity() * 100));
				}
				if (rn.getNutrientCode().equals(NutriProfiler.NutriEnum.FIBER.getNutrientCode())
						&& NutriByte.person.dietNutrientsMap.containsKey(rn.getNutrientCode())) {
					actualDataList.get(3).setYValue(
							Math.round(NutriByte.person.dietNutrientsMap.get(rn.getNutrientCode()).getNutrientQuantity()
									/ rn.getNutrientQuantity() * 100));
				}
			}
	...
	// setupNutriChart is called only once from View to create and bind all the
	// elements together
	// There are only four nutrients to be charted with recommended and actual
	// values
	...
	void setupNutriChart() {
		...
		// add placeholder for actual value for Energy as 0 quantity, create data label,
		// and pass them to displayLableForData() method for formatting.
		final XYChart.Data<String, Number> actualEnergyData = new Data<String, Number>(
				Model.nutrientsMap.get(NutriProfiler.ENERGY_NUTRIENT_CODE).getNutrientName(), 0);
		final Text energyDataText = new Text();
		energyDataText.textProperty().bind(Bindings.format("%s%%", actualEnergyData.YValueProperty()));
		actualEnergyData.nodeProperty()
				.addListener((observable, oldValue, newValue) -> displayLabelForData(actualEnergyData, energyDataText));
		actualDataList.add(actualEnergyData);

		// Protein actual data
		final XYChart.Data<String, Number> actualProteinData = new Data<String, Number>(
				Model.nutrientsMap.get(NutriProfiler.NutriEnum.PROTEIN.getNutrientCode()).getNutrientName(), 0);
		final Text proteinDataText = new Text();
		proteinDataText.textProperty().bind(Bindings.format("%s%%", actualProteinData.YValueProperty()));
		actualProteinData.nodeProperty().addListener(
				(observable, oldValue, newValue) -> displayLabelForData(actualProteinData, proteinDataText));
		actualDataList.add(actualProteinData);

		// Carbohydrate actual data
		final XYChart.Data<String, Number> actualCarbohydrateData = new Data<String, Number>(
				Model.nutrientsMap.get(NutriProfiler.NutriEnum.CARBOHYDRATE.getNutrientCode()).getNutrientName(), 0);
		...
		actualCarbohydrateData.nodeProperty().addListener(
				(observable, oldValue, newValue) -> displayLabelForData(actualCarbohydrateData, carbohydrateDataText));
		actualDataList.add(actualCarbohydrateData);

		// Fiber actual data
		final XYChart.Data<String, Number> actualFiberData = new Data<String, Number>(
				Model.nutrientsMap.get(NutriProfiler.NutriEnum.FIBER.getNutrientCode()).getNutrientName(), 0);
		final Text fiberDataText = new Text();
		fiberDataText.textProperty().bind(Bindings.format("%s%%", actualFiberData.YValueProperty()));
		actualFiberData.nodeProperty()
				.addListener((observable, oldValue, newValue) -> displayLabelForData(actualFiberData, fiberDataText));
		actualDataList.add(actualFiberData);

		// add recommended value and category names for Energy nutrient
		recommendedDataList.add(new Data<String, Number>(
				Model.nutrientsMap.get(NutriProfiler.ENERGY_NUTRIENT_CODE).getNutrientName(), 100));
		...
		for (NutriProfiler.NutriEnum nutriEnum : NutriProfiler.NutriEnum.values()) {
			...
			case FIBER: {
				recommendedDataList.add(new Data<String, Number>(
						Model.nutrientsMap.get(nutriEnum.getNutrientCode()).getNutrientName(), 100));
