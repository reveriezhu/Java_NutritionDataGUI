...
public class NutriProfiler {
    ...
    /*populate nutriRequirement and energyRequirement for the person passed into the method
     * instantiate a RecommendedNutrient for each nutrient(including energy),
     * and then add it into NutriProfiler.recommendedNutrientsList, a static list
     **/
	static void createNutriProfile(Person person) {

		float[] nutriReq = person.calculateNutriRequirement();
		float energyReq = person.calculateEnergyRequirement();

		//populating the observable list, NutriProfiler.recommendedNutrientsList
		person.recommendedNutrientsList.clear();//clear the static list first.
		NutriEnum[] ne = NutriEnum.values();// read all values in  NutriEnum datatype into a NutriEnum[]
		//populate energy
		person.recommendedNutrientsList.add(new RecommendedNutrient(ENERGY_NUTRIENT_CODE, energyReq));

		// populate nutrients
		for(int i=0;i< nutriReq.length;i++){
			person.recommendedNutrientsList.add(new RecommendedNutrient(ne[i].getNutrientCode(), nutriReq[i]));
		}
